...
class CachedFile:
    def __init__(self, file, path=None, cache_dir=None, block_size=DEFAULT_BLOCK_SIZE, data_file=None, mask_file=None, read_as_buffer=True):
        ...
        The standard location for the cache is: `${VAEX_PATH_CACHE_FS}/<protocol (e.g. s3)>/path/to/file.ext`
        ...
        :cache_dir str: path of cache dir, defaults to `${VAEX_PATH_CACHE_FS}`
        """
        self.name = path
        self.path = path
        self.file = file
        ...
        if data_file is None or mask_file is None:
            ...
            if cache_dir is None:
                self.cache_dir_path = os.path.join(vaex.settings.paths.cache_fs, o.scheme, o.netloc, o.path[1:])
